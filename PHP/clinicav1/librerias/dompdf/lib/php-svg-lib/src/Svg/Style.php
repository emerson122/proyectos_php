...
class Style
{
    ...
    public function fromAttributes($attributes)
    {
        ...
        if (isset($attributes["style"])) {
            $ = self::parseCssStyle($attributes["style"]);
            $this->fill($);
        }
    }
    ...
    public function fromheets(AbstractTag $tag, $attributes) {
        $class = isset($attributes["class"]) ? preg_split('/\s+/', trim($attributes["class"])) : null;

        $heets = $tag->getDocument()->getheets();

        $ = array();

        foreach ($heets as $_sc) {
            ...
            if (isset($[$from])) {
                ...
                switch ($type) {
                    case self::TYPE_COLOR:
                        $value = self::parseColor($[$from]);
                        break;

                    case self::TYPE_NUMBER:
                        $value = ($[$from] === null) ? null : (float)$[$from];
                        break;
